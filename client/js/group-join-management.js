// client/js/group-join-management.js
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadRequests = async () => {
        ...
        try {
            ...
            if (requests.length > 0) {
                ...
                listContainer.innerHTML = '<tr><td colspan="4" class="info-message">No pending group join requests.</td></tr>';
            }
        } catch (error) {
    ...
    listContainer.addEventListener('click', async (e) => {
        ...
        if (action) {
            try {
                await window.api.post(`/admin/group-join-requests/${requestId}`, { action });
